...
pub async fn show_last_order(opts: &AppOptions) {
    let client_id = match get_client_id(opts) {
        Some(id) => id,
        None => return,
    };
    ...
    match api::fetch_pending_orders(&ctx, client_id).await {
        Ok(orders) => {
            ...
            match api::fetch_order_detail(&ctx, &order.uid).await {
                ...
                Err(e) => {
                    drop(sp2);
                    // Fallback: show basic info from pending order
                    println!(
                        "  Pedido #{} - {} - {}",
        ...
        Err(e) => {
            drop(sp);
            eprintln!("Erro ao buscar pedidos: {}", e);
        }
    }
}
...
fn get_client_id(opts: &AppOptions) -> Option<u64> {
    if opts.stateless {
        println!(
            "{}",
